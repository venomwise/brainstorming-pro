...
- `/brainstorm-pro "<request>"` — start a new runtime-managed workflow. The command binds a provider-qualified workflow agent model from the current Pi session model, or runs `pi --list-models`, parses its provider/model rows, and prompts with those configured Pi models when no session model is selected. The selected provider and model are formatted internally as `<provider>/<model>` with the provider value preserved exactly, then persisted as the string `agentModel` in `specs/<topic>/.workflow/runs/<run-id>/state.json`; the runtime enters `designing`.
- `/brainstorm-pro "<request>" --topic <existing-topic>` — continue an existing brainstorm with the current `design.md` as background context. This creates a new run for the same topic, records the supplemental request, preserves the workflow agent model, resets design review/approval state, and returns to `designing`.
- `/brainstorm-pro --topic <existing-topic>` — resume an existing workflow by topic.
- `/brainstorm-pro --resume [topic]` — resume the next runtime-managed workflow step. Resume is state-aware: it reuses the workflow's persisted `agentModel` instead of substituting the current session model, displays review choices at review decision gates, displays approval choices at approval gates, returns blocked/failed states fail-closed, and never silently chooses review depth or approval.
- `/brainstorm-pro --status [topic]` — show runtime phase, pending decision, latest artifact refs, review status, and last error for a runtime-managed workflow without prompting for or patching a workflow model.

If no selected model is available in a non-interactive or unconfigured environment, `/brainstorm-pro` fails before workflow creation with setup guidance to inspect `pi --list-models` output, fix Pi model configuration, or start Pi with `--model provider/model`. Legacy workflows missing `agentModel` are patched once at command-layer resume after the same model resolution step; lower runtime and adapter layers fail closed when an agent-backed phase is invoked without a valid provider-qualified model. `agentModel` remains a string, and Brainstorming Pro treats `<provider>/<model>` as its persisted and child-CLI argument format rather than as the literal output format of `pi --list-models`.

The resume/status output is rendered by a deterministic state-aware UX layer: it shows workflow selection hints, topic/run/phase summaries, pending decisions, exact artifact refs/checksums, review coverage/readiness, revision handoff, automatic plan review diagnostics, blocked/failed diagnostics, and terminal status without mutating workflow files or advancing state by itself. Foreground resume can also attach a live progress presentation when TUI support is available; optional interactive controls submit intent through the runtime decision facade with gate nonce, artifact/checksum binding, explicit confirmations, and idempotency keys. Non-interactive, stale, narrow, unsupported, or failed TUI contexts use deterministic text fallback and `/brainstorm-pro --resume <topic>` hints. Safe next-command hints stay within the same public command surface, for example `/brainstorm-pro --resume <topic>` and `/brainstorm-pro --status <topic>`.

Internal helper flags such as `--choose-review` and `--decision` are advanced shortcuts accepted only with `--resume`; they are not a replacement for the state-aware gate model and cannot bypass runtime validation. Helper decisions are passed to runtime-owned decision types. Plan review is automatic and fixed: there is no user-selected plan review `skip`, `minimal`, or `full` mode helper.
