...
- `/brainstorm-pro "<request>"` — start a new runtime-managed workflow. The command binds a provider-qualified workflow agent model from the current Pi session model, or prompts with configured models when no session model is selected. The runtime asks that selected LLM to summarize the request into a safe English kebab-case topic, creates `specs/<topic>/.workflow/runs/<run-id>/state.json` with the selected `agentModel`, and enters `designing`.
