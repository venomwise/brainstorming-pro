...
This repository is a Pi package for structured requirement clarification workflows. Core TypeScript lives in `extensions/clarification-orchestrator/`, with command handlers in `commands/`, the durable `/brainstorm-pro` runtime under `workflow/` (`types.ts`, `state-machine.ts`, `runtime.ts`, workflow artifact/event/gate helpers, `workflow/adapters/*`, and small workflow persistence helpers), read-only TUI/status helpers under `tui/`, and future foreground child execution scaffolding under `runtime/agent-execution/`. Infrastructure-only reuse metadata for `nicobailon/pi-subagents` lives under `extensions/clarification-orchestrator/vendor/pi-subagents/` (`reuse-inventory.json`, `LICENSE`, `NOTICE.md`, and policy README); executable derived helpers must live in Brainstorming Pro-owned source directories, not under `vendor/`. Shared modules include `options.ts`, `path-guard.ts`, and `topic-validation.ts`. Packaged Pi skills are in `skills/*/SKILL.md`. Tests are grouped by scope under `tests/unit/`, `tests/integration/`, and `tests/security/`, with workflow runtime unit tests in `tests/unit/workflow/`, runtime command tests in `tests/unit/commands/`, documentation alignment tests in `tests/unit/docs/`, pi-subagents reuse validation tests in `tests/unit/*pi-subagents*.test.ts` and `tests/security/pi-subagents-*.test.ts`, and reusable sample data in `tests/fixtures/`. Specs and generated clarification artifacts belong under `specs/<topic>/`.
